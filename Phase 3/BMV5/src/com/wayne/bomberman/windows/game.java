/**
 * game class. This class is the main. It includes loading level objects and state changes
 */
...
public class game extends Canvas implements Runnable{
	...
	public void run()
	{
		...
		while(running){
			...
			while(time >= 1){
				tick();	
				time--;
				//time decrements when in game state
				if(State==STATE.GAME){
					timeLeft--;
	...
	private void tick()
	{
		//update handler only when in game
		if(State==STATE.GAME)
		{
			handler.tick();
		}
	...
	/**
	 * rendering method for all states
	 */
	private void render()
	{
	...
	/**
	 * this methods draws the level. colors detected create the associated object
	 * @param image file to be read
	 */
	private void LoadImageLevel(BufferedImage image)
	{
		if(State==STATE.GAME)
		{
			//only for testing, checks for level file dimension size
